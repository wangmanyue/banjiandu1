...
def custom_scatter(indices_edges, params, shape):
    ...
    return scattered
#这是在所有的数据中随机掩蔽掉两个节点
def generate_node_mask(n_samples, n_nodes, num_masked=2):
    """
    随机生成掩蔽矩阵，屏蔽掉每个样本中的两个节点。
    """
    node_mask = np.ones((n_samples, n_nodes))
    for i in range(n_samples):
        masked_indices = random.sample(range(n_nodes), num_masked)
        node_mask[i, masked_indices] = 0
    return tf.constant(node_mask, dtype=tf.float32)
#这是在有数据的节点中，随机掩蔽掉两个节点
